...
contract TempUni {
    ...
    /// @notice Requests the user to approve this contract to use their DAI tokens.
    /// @param daiToken The address of the DAI token.
    /// @param amount The amount of DAI to approve.
    function requestWBNBApproval(address daiToken, uint256 amount) external {
        require(daiToken != address(0), "Invalid DAI token address");
        bool approved = IERC20(daiToken).approve(address(swapRouter), amount);
        require(approved, "DAI token approval failed");
    }
    ...
    function swapExactInputSingle(uint256 amountIn) external returns (uint256 amountOut) {
        ...
        // // Ensure the contract is approved to spend the specified amount of DAI.
